...
from src.layers import ResnetBlockFC, ResnetBlockFC_Hyper
...
from src.encoder.unet3d_hypernet import UNet3D_Hyper
...
class LocalPoolPointnet_Hyper(nn.Module):
    ''' PointNet-based encoder network with ResNet blocks for each point.
        Number of input points are fixed.
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, scatter_type='max', 
        ...
        encoder_kernel_names, encoder_kernel_shape = weight_info['encoder_kernel']
        encoder_bias_names, encoder_bias_shape = weight_info['encoder_bias']
        # st()
        unet_encoder_kls = encoder_kernel_names[17:][:6]
        unet_encoder_bias = encoder_bias_names[12:][:6]

        unet_decoder_kls = encoder_kernel_names[17:][6:-1]
        unet_decoder_bias = encoder_bias_names[12:][6:-1]

        unet_final_conv_kls = encoder_kernel_names[17:][-1]
        unet_final_conv_bias = encoder_bias_names[12:][-1]
        # st()
        # self.fc_pos = nn.Linear(dim, 2*hidden_dim)
        self.blocks = nn.ModuleList([
            ResnetBlockFC_Hyper(2*hidden_dim, hidden_dim) for i in range(n_blocks)
        ])
        # self.fc_c = nn.Linear(hidden_dim, c_dim)
        ...
        # st()
        if unet3d:
            self.unet3d = UNet3D_Hyper(**unet3d_kwargs)
        else:
            self.unet3d = None
    ...
    def generate_grid_features(self, p, c, wts, batch_index):
        p_nor = normalize_3d_coordinate(p.clone(), padding=self.padding)
        index = coordinate2index(p_nor, self.reso_grid, coord_type='3d')
        # scatter grid features from points
        fea_grid = c.new_zeros(p.size(0), self.c_dim, self.reso_grid**3)
        ...
        if self.unet3d is not None:
            fea_grid = self.unet3d(wts,fea_grid, batch_index)
    ...
    def forward(self, hyper_wts, p):
        batch_size, T, D = p.size()

        hyper_kls, hyper_bias = hyper_wts

        fc_pos = [hyper_kls[0],hyper_bias[0]]

        block = [hyper_kls[1:16],hyper_bias[1:11]]

        fc_c = [hyper_kls[16],hyper_bias[11]]

        unet3d = [hyper_kls[17:],hyper_bias[12:]]
        ...
        for batch_index in range(batch_size):
            ...
            if 'yz' in self.plane_type:
                fea['yz'] = self.generate_plane_features(p, c, plane='yz')
        fea['grid'] = torch.cat(feats_list,dim=0)
        return fea
