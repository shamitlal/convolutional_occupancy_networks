...
def create_conv(hyper_wt, x,in_channels, out_channels, kernel_size, order, num_groups,batch_index, padding=1):
    """
    ...
    weight, bias = hyper_wt
    # modules = []
    for i, char in enumerate(order):
        if char == 'r':
            x = nn.ReLU(inplace=True)(x)
        elif char == 'l':
            x = nn.LeakyReLU(negative_slope=0.1, inplace=True)(x)
        elif char == 'e':
            x = nn.ELU(inplace=True)(x)
        elif char == 'c':
            # add learnable bias only in the absence of batchnorm/groupnorm
            # bias = not ('g' in order or 'b' in order)
            x = F.conv3d(x, weight[batch_index], bias=bias[batch_index],padding=padding)
        elif char == 'g':
            pass
        ...
        #     x = nn.GroupNorm(num_groups=num_groups, num_channels=num_channels)(x)
        elif char == 'b':
            pass
        #     is_before_conv = i < order.index('c')
        #     if is_before_conv:
...
def SingleConv(hyper_wts, batch_index, x, in_channels, out_channels, kernel_size=3, order='crg', num_groups=8, padding=1):
    """
    ...
    Args:
        ...
        num_groups (int): number of groups for the GroupNorm
    """
    x = create_conv(hyper_wts, x, in_channels, out_channels, kernel_size, order, num_groups, batch_index, padding=padding)
    return x
...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='crg', num_groups=8):
        ...
        if encoder:
            ...
            self.conv1_out_channels = out_channels // 2
            if self.conv1_out_channels < in_channels:
                self.conv1_out_channels = in_channels
            self.conv2_in_channels, self.conv2_out_channels = self.conv1_out_channels, out_channels
        else:
            # we're in the decoder path, decrease the number of channels in the 1st convolution
            self.conv1_in_channels, self.conv1_out_channels = in_channels, out_channels
            self.conv2_in_channels, self.conv2_out_channels = out_channels, out_channels

    def forward(self,hyper_wts, x, batch_index):
        singleconv_kernel, singleconv_bias = hyper_wts
        # conv1
        x = SingleConv([singleconv_kernel[0], singleconv_bias[0]], batch_index, x, self.conv1_in_channels, self.conv1_out_channels, self.kernel_size, self.order, self.num_groups)
        # conv2
        x = SingleConv([singleconv_kernel[1], singleconv_bias[1]], batch_index, x, self.conv2_in_channels, self.conv2_out_channels, self.kernel_size, self.order, self.num_groups)
        return x
...
class Encoder(nn.Module):
    ...
    def forward(self, hyperwts, x, batch_index):
        if self.pooling is not None:
            x = self.pooling(x)
        x = self.basic_module(hyperwts,x,batch_index)
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, hyperwts, encoder_features, x, batch_index):
...
class Abstract3DUNet_Hyper(nn.Module):
    """
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
        ...
        super(Abstract3DUNet_Hyper, self).__init__()
    ...
    def forward(self, hyperwts, x, batch_index):
        hyperwts_kls,hyperwts_bias = hyperwts

        encoder_kls = hyperwts_kls[:6]
        encoder_bias = hyperwts_bias[:6]

        decoder_kls = hyperwts_kls[6:-1]
        decoder_bias = hyperwts_bias[6:-1]

        final_conv_kls = hyperwts_kls[-1]
        final_conv_bias = hyperwts_bias[-1]
        ...
        for ind,encoder in enumerate(self.encoders):
            x = encoder([encoder_kls[ind*2:(ind+1)*2],encoder_bias[ind*2:(ind+1)*2]],x,batch_index)
        ...
        for decoder, encoder_features in zip(self.decoders, encoders_features):
            ...
            x = decoder([decoder_kls[ind*2:(ind+1)*2],decoder_bias[ind*2:(ind+1)*2]],encoder_features, x,batch_index)
            ind = ind+1 
        # st()
        x = F.conv3d(x, final_conv_kls[batch_index], bias=final_conv_bias[batch_index])
...
class UNet3D_Hyper(Abstract3DUNet_Hyper):
    """
    3DUnet model from
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
        ...
        super(UNet3D_Hyper, self).__init__(in_channels=in_channels, out_channels=out_channels, final_sigmoid=final_sigmoid,
...
class ResidualUNet3D(Abstract3DUNet_Hyper):
    """
